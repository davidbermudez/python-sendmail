...
def render_template(archivo_csv):
    ...
    plantilla_html = leer_plantilla_html('base copy 2.html')
    nombre = '1'
    while(nombre!=''):
        nombre = ''
        ...
        if(nombre!='' and email_!='EMAIL'):            
            mensaje_html = plantilla_html.format(
                ...
                article=f'<p>Hola {nombre},<br><br>Queremos recordarte que el curso "{curso_pf}" dará comienzo el próximo <b>21 de abril</b> y finalizará el <b>25 de mayo</b>. Se trata de un curso tutorizado con seguimiento, \
                    por lo que es importante que te ajustes a estas fechas para poder seguir correctamente su desarrollo.</p> \
                    <p>Desde ISFES se pondrán en contacto contigo antes del inicio del curso para facilitarte las instrucciones de acceso. \
                    Te recomendamos que revises con frecuencia tu bandeja de entrada, así como la de <b>correo no deseado o spam</b>, por si el mensaje llegara allí. En caso de no haber recibido dicha comunicación llegado el momento, puedes ponerte en contacto con ellos a través del siguiente correo electrónico</p> \
                    <ul><li>aula@isfes.es</li></ul><p>Esperamos que disfrutes del curso y que te resulte útil e interesante.</p> \
            ...
            # Solo curso_pf
            if(curso_pf!=''):
                if (enviar_email(email_, mensaje_html, imagen_oculta_url, nombre + ' tu curso comienza el 21 de abril')):
...
if __name__ == "__main__":
    archivo_csv = 'Vehiculos-pruebas.csv'
